...
interface WeatherData {
  ...
  sys: { country: string };
  visibility: number;
  rain?: { "1h"?: number; "3h"?: number }; // Rain data (optional)
}
...
const WeatherAlert: React.FC = () => {
  ...
  return (
    <div className="absolute top-24 right-6 bg-white shadow-lg p-6 rounded-md border border-gray-300">
      {error ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          {weatherData.rain ? (
            <p>
              ...
              {weatherData.rain["1h"]
                ? `${weatherData.rain["1h"]} mm (last 1h)`
                : ""}
              {weatherData.rain["3h"]
                ? `${weatherData.rain["3h"]} mm (last 3h)`
                : ""}
            </p>
          ) : (
            <p>
              <strong>Rainfall:</strong> 0 mm
            </p>
          )}
        </div>
      ) : (
